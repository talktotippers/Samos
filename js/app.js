(() => {
  ...
  function renderToday(){
    ...
    return `
      ...
      <section class="section grid">
        ...
        ${homeTile("🇬🇷","Greek Guide","Translator, phrases and menu decoder","guide")}
  ...
  function renderEatDrink(){
    ...
    return `
      <section class="hero" style="background-image:linear-gradient(135deg,rgba(23,107,115,.88),rgba(14,75,82,.88)),url('https://source.unsplash.com/1400x900/?greek,food,taverna')">
        ...
        <div class="hero-actions"><button class="secondary" data-view="guide">Open Greek guide and translator</button></div>
      </section>
  ...
  function renderGuide(){
    return `
      <section class="hero" style="background-image:linear-gradient(135deg,rgba(23,107,115,.88),rgba(14,75,82,.9)),url('https://source.unsplash.com/1400x900/?greek,menu')">
        <p class="eyebrow">Translator and menu help</p>
        <h1>Greek Guide</h1>
        <p>Search useful phrases, menu words, dishes, beer, wine, petrol and child friendly requests. Every phrase has copy and a Google Translate link.</p>
      </section>

      <section class="section card pad">
        <div class="section-head"><div><p class="eyebrow">Translator</p><h2>Phrasebook search</h2><p class="lead">Try searches like bill, beer, petrol, toilet, pharmacy, high chair, water or recommendation.</p></div></div>
        <div class="searchbar"><input id="phraseSearch" placeholder="Search English, Greek, pronunciation or situation..." /></div>
        <div id="phraseResults" class="grid">${DATA.phrases.map(phraseCard).join("")}</div>
      </section>

      <section class="section card pad">
        <div class="section-head"><div><p class="eyebrow">Menus</p><h2>Local dishes and what to look for</h2><p class="lead">Use this when you are staring at a menu and want to know what is worth ordering.</p></div></div>
        <div class="searchbar"><input id="dishSearch" placeholder="Search octopus, Muscat, cheese, prawns, beer..." /></div>
        <div id="dishResults" class="grid">${DATA.dishes.map(dishCard).join("")}</div>
      </section>
    `;
  }
  ...
  function bindViewEvents(){
    ...
    $all("[data-copy]").forEach(btn => btn.addEventListener("click", async () => {
      ...
      catch { const t=document.createElement("textarea"); t.value=btn.dataset.copy; document.body.appendChild(t); t.select(); document.execCommand("copy"); t.remove(); }
  ...
  function toggle(kind, id){
    const bucket = {favourite:"favourites", visited:"visited", highlight:"highlights"}[kind] || kind;
